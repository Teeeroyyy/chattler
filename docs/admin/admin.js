...
(() => {
  ...
  const STATUS = { ok: 'On the list', denied: 'No access', owner: 'Owner' };

  function filtered() {
    const q = $('[data-search]').value.trim().toLowerCase();
    const f = $('[data-filter]').value;
    return users.filter((u) => (!q || u.login.includes(q))
      && (f === 'all' || (f === 'active' ? Date.now() - new Date(u.last_seen) < 7 * DAY : u.status === f)));
  }
  ...
  function render() {
    ...
    $('[data-stats]').innerHTML = [
      ...
      [denied, 'tried without access'],
    ].map(([n, l]) => `<div><b>${n}</b><span>${l}</span></div>`).join('');

    const list = filtered().sort((a, b) => {
      const k = sort.key;
      const d = k === 'sessions' ? a[k] - b[k] : k === 'version' ? cmpVer(a[k], b[k]) : String(a[k] || '').localeCompare(String(b[k] || ''));
      return sort.asc ? d : -d;
    });
    $('[data-rows]').innerHTML = list.map((u) => `
      <tr>
        <td><a href="https://twitch.tv/${esc(u.login)}" target="_blank" rel="noopener">${esc(u.login)}</a></td>
        <td><span class="pill ${esc(u.status)}">${esc(STATUS[u.status] || u.status)}</span></td>
  ...
  function exportCsv() {
    const rows = [['twitch_name', 'access', 'first_login', 'last_opened', 'version', 'opens'],
      ...filtered().map((u) => [u.login, u.status, u.first_seen, u.last_seen, u.version || '', u.sessions])];
